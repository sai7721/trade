...
def scalping_trade(coinString, coin):
    ...
    if 20000 >= coinAskPrice[coinString] > 16000:
        maxPrice = 20000
        minPrice = 16000
        priceGap = 50
    elif 15000 >= coinAskPrice[coinString] > 9600:
        maxPrice = 15000
        minPrice = 9600
        priceGap = 30
    elif 9000 >= coinAskPrice[coinString] > 6400:
        maxPrice = 9000
        minPrice = 6400
        priceGap = 20
    elif 6000 >= coinAskPrice[coinString] > 4300:
        maxPrice = 6000
        minPrice = 4300
        priceGap = 15
    elif 4000 >= coinAskPrice[coinString] > 2200:
        maxPrice = 4000
        minPrice = 2200
        priceGap = 10
    elif 2000 >= coinAskPrice[coinString] > 1000:
        maxPrice = 2000
        minPrice = 1000
        priceGap = 5
            ...
            for i in range(count): # 0 ~ count -1 까지
                ...
                if buyLimitOrderState == "done":
                    ...
                    if sellLimitOrderState == "None": # 매도 정보가 없으면
                        ...
                        # 보유 수량이 주문할 수량보다 많으면, 주문할 수량으로 매도 주문한다.
                        if coinOrderBidVolume[coinString][searchCount] <= balance:
                        ...
                        else: # 보유 수량이 매도 주문할 수량보다 적다면, 전량을 매도 주문한다.
                            coinSellLimitOrder[coinString][searchCount] = upbit.sell_limit_order("KRW-" + coin, coinOrderBidPrice[coinString][searchCount] + priceGap, balance)
